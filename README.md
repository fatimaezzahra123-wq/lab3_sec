...
### Étape 3 — Adresse réseau utilisée
...
> 💡 `10.0.2.2` est l'adresse "magique" de l'émulateur Android qui pointe vers `localhost` de la machine hôte.
...
- Aucun cookie de session sur `testphp.vulnweb.com` (première visite)
- Header `Upgrade-Insecure-Requests: 1` présent
...
1. **Forcer HTTPS** — utiliser `Network Security Config` Android
2. **Sécuriser les cookies** — attributs `HttpOnly`, `Secure`, `SameSite=Strict`
3. **Minimiser les données dans les URLs** — éviter les paramètres sensibles
4. **Certificate Pinning** — pour les applis sensibles

---

## 🧹 Fin de lab — Nettoyage

1. **Désactiver le proxy** → Settings → Wi-Fi → Proxy: **None**
2. **Retirer tout certificat** → Settings → Security → Clear credentials
